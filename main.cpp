...
vec3 findColor(Scene& scene, Ray& ray, int depth) {
	...
	Ray reflectedRay = Ray(hit.point+EPSILON*normal, ray.direction+(2.0f*normal*c1));
...
void raytrace(Scene& scene) {
	...
	if (!bitmap) exit(1);
	float subdivisions = scene.antialias;
	float subdivide = 1/subdivisions;
	...
	for (int j=0; j<scene.height; j++){
		...
		for (int i=0; i<scene.width; i++) {			
			vec3 color;
			for(float a=0; a<subdivisions; a+=1) {
				for(float b=0; b<subdivisions; b+=1) {
					float randomNum1 = ((float)rand()/(float)RAND_MAX) * subdivide;
					float randomNum2 = ((float)rand()/(float)RAND_MAX) * subdivide;
			...
			rgb.rgbRed = min(color[0],1.0f)*255.0f;
			rgb.rgbGreen = min(color[1],1.0f)*255.0f;
			rgb.rgbBlue = min(color[2],1.0f)*255.0f;
